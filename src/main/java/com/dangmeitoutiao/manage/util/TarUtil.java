...
public class TarUtil {
    ...
    private final static String SPLIT = System.getProperty("file.separator");
     ...
     * @param source 源文件
     */
    public static void tarGz(File source) {
        File tarDest = new File(source.getPath() + ".tar");
        tar(source, tarDest);
        gzip(tarDest);
    }
    ...
    public static void tarDir(File sourceDir, TarArchiveOutputStream tarOut, String parentPath) {
        ...
        if (sourceDir.listFiles().length < 1) {
            TarArchiveEntry entry = new TarArchiveEntry(parentPath + sourceDir.getName() + SPLIT);
            try {
                tarOut.putArchiveEntry(entry);
                tarOut.closeArchiveEntry();
            } catch (IOException e) {
        ...
        for (File file : sourceDir.listFiles()) {
            tarPack(file, tarOut, parentPath + sourceDir.getName() + SPLIT);
        }
    }
...
//        TarUtil.unTarGz(new File("C:\\Users\\zhoukai\\Desktop\\spider\\SiteManage\\data\\127.0.0.1.tar.gz"), new File("C:\\Users\\zhoukai\\Desktop\\spider\\SiteManage\\data\\127.0.0.1"));
        TarUtil.tarGz(new File("C:\\Users\\zhoukai\\Desktop\\spider\\spider\\spider3\\data\\sites"));
    }
    ...
    public static void gzip(File source) {
        ...
        int lastIndexOf = sourcePath.lastIndexOf(SPLIT);
        String dir = sourcePath.substring(0, lastIndexOf);
        File target = new File(dir + SPLIT + source.getName() + ".gz");
        FileInputStream fis = null;
